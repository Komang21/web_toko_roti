...
<div class="p-6 max-w-md mx-auto">
    <h2 class="text-2xl font-semibold mb-6">Tambah Bahan Baku Baru</h2>
    ...
    <form action="{{ route('bahan-baku.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="harga" value="{{ old('harga') }}" step="0.01" class="shadow appearance-none border rounded w-full py-2 px-3 @error('harga') border-red-500 @enderror" required>
            @error('harga')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
