            ...
            <div class="p-6 max-w-2xl mx-auto bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('supplier.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="alamat" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 @error('alamat') border-red-500 @enderror" required>{{ old('alamat') }}</textarea>
                        @error('alamat')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-6">
                        ...
                        <input type="text" name="telp" value="{{ old('telp') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 @error('telp') border-red-500 @enderror">
                        @error('telp')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
